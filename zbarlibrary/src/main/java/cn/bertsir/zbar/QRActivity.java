...
import cn.bertsir.zbar.utils.UrlParse;
...
//        Log.i("zBarLibary", "version: "+BuildConfig.VERSION_NAME);
    ...
    private void initView() {
        ...
        ivLeft = findViewById(R.id.iv_close);
    ...
    private void scanFinish(ScanResult result) {
        QrManager.getInstance().getResultCallback().onScanSuccess(result);
        String scanResult = result.getContent();

        String hostPath = UrlParse.getUrlHostAndPath(scanResult);
        Map<String, String> params = UrlParse.getUrlParams(scanResult);

        String schemeMacAddress = params.get("m");
        String schemeDeviceName = params.get("n");

        if (!TextUtils.isEmpty(schemeMacAddress) && !TextUtils.isEmpty(schemeDeviceName)) {
            finish();
        }
    }
